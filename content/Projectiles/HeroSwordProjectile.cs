...
namespace TheLegendofTerra.Content.Projectiles
{
    ...
    public class HeroSwordProjectile : ModProjectile
    {
        ...
        private bool canShootBeams = true;

        private int beamChargeUp = 0;
        ...
        public override void AI()
        {
            ...
            if (player.statLife < (player.statLifeMax * 0.9) && beamChargeUp < 1000)
                {
                canShootBeams = false;
            }
            ...
            if (canShootBeams)
            {
               int a = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, new Vector2 (Projectile.velocity.X, Projectile.velocity.Y), ModContent.ProjectileType<HeroSwordBeam>(), 14, 1f, Main.myPlayer);
                Main.projectile[a].velocity = Projectile.velocity * 5;
                SoundEngine.PlaySound(new SoundStyle("TheLegendofTerra/Content/Sounds/heroswordbeam"));
                beamChargeUp = 0;
                canShootBeams = false;
                    }
